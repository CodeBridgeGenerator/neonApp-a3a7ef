...
describe("orderProduct service", async () => {
  ...
  describe("#create", () => {
    const options = {
      ...
      quantity: 23,
      selectedColor: "new value",
      selectedSize: "new value",
      unitPrice: 23,
    };

    beforeEach(async () => {
      orderProductCreated = await thisService.Model.create({
        ...options,
        ...users,
      });
    });
    ...
    it("should create a new orderProduct", () => {
      ...
      assert.strictEqual(
        orderProductCreated.selectedColor,
        options.selectedColor,
      );
      assert.strictEqual(
        orderProductCreated.selectedSize,
  ...
  describe("#get", () => {
    it("should retrieve a orderProduct by ID", async () => {
      ...
      assert.strictEqual(
        ...
        orderProductCreated._id.toString(),
      );
    });
  });
  ...
  describe("#update", () => {
    const options = {
      ...
      quantity: 100,
      selectedColor: "updated value",
      selectedSize: "updated value",
      unitPrice: 100,
    };
    ...
    it("should update an existing orderProduct ", async () => {
      ...
      assert.strictEqual(
        orderProductUpdated.selectedColor,
        options.selectedColor,
      );
      assert.strictEqual(
        orderProductUpdated.selectedSize,
        options.selectedSize,
      );
  ...
  describe("#delete", async () => {
    it("should delete a orderProduct", async () => {
      ...
      const orderProductDeleted = await thisService.Model.findByIdAndDelete(
        orderProductCreated._id,
      );
      assert.strictEqual(
        orderProductDeleted._id.toString(),
        orderProductCreated._id.toString(),
      );
    });
  });
