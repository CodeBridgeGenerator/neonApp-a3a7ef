...
const colour = Array.isArray(res.colour)
            ? res.colour.map((elem) => ({ _id: elem._id, colorName: elem.colorName }))
            : res.colour
                ? [{ _id: res.colour._id, colorName: res.colour.colorName }]
                : [];
        setColour(colour);
const size = Array.isArray(res.size)
            ? res.size.map((elem) => ({ _id: elem._id, sizeValue: elem.sizeValue }))
            : res.size
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Colour</label>
                    {colour.map((elem) => (
