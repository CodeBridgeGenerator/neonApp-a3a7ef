...
const CheckoutCreateDialogComponent = (props) => {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const eagerResult = await client.service("checkout").find({
        query: {
          $limit: 10000,
          _id: { $in: [result._id] },
          $populate: [
      ...
      props.alert({
        type: "success",
        title: "Create info",
        message: "Info Checkout updated successfully",
      });
      ...
      props.alert({
        type: "error",
        title: "Create",
        message: "Failed to create in Checkout",
      });
    }
    setLoading(false);
  };
  ...
  return (
    ...
    >
      <div
        ...
        style={{ maxWidth: "55vw" }}
        role="checkout-create-dialog-component"
      >
        ...
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <Dropdown
              id="productName"
              value={_entity?.productName?._id}
              optionLabel="name"
              optionValue="value"
              options={productNameOptions}
              onChange={(e) => setValByKey("productName", { _id: e.value })}
            />
          </span>
          <small className="p-error">
