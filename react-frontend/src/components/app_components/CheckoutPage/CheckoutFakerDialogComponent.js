...
import checkoutFakerFactory from "./checkoutFakerFactory";
...
const CheckoutFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [productNameIds, setproductNameIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("product")
                        .find({ query: { $select: ["_id","productTitle","description","price","productImage"] } })
                        .then((data) => {
                        setproductNameIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = checkoutFakerFactory(props.user,fakerCount,productNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("checkout").create(elem));
...
export default connect(mapState, mapDispatch)(CheckoutFakerDialogComponent);
