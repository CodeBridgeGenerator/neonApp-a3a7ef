...
const CustomerAddressCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            customerName: _entity?.customerName,addressType: _entity?.addressType,country: _entity?.country,addressLine1: _entity?.addressLine1,addressLine2: _entity?.addressLine2,city: _entity?.city,postalCode: _entity?.postalCode,
