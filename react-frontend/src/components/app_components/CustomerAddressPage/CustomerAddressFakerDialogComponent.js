...
import customerAddressFakerFactory from "./customerAddressFakerFactory";
...
const CustomerAddressFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = customerAddressFakerFactory(props.user, fakerCount);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("customerAddress").create(elem),
    );
