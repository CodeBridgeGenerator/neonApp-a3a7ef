...
import CustomerAddressDatatable from "./CustomerAddressDataTable";
import CustomerAddressEditDialogComponent from "./CustomerAddressEditDialogComponent";
import CustomerAddressCreateDialogComponent from "./CustomerAddressCreateDialogComponent";
import CustomerAddressFakerDialogComponent from "./CustomerAddressFakerDialogComponent";
import CustomerAddressSeederDialogComponent from "./CustomerAddressSeederDialogComponent";
...
const CustomerAddressPage = (props) => {
  ...
  const filename = "customerAddress";
  ...
  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("customerAddress");
    ...
    client
      ...
      .catch((error) => {
        ...
        props.alert({
          ...
          type: "error",
          message: error.message || "Failed get Customer Address",
        });
      });
  ...
  const deleteRow = async () => {
    try {
      await client.service("customerAddress").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("customerAddress").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  const onRowClick = ({ data }) => {
    navigate(`/customerAddress/${data._id}`);
  };
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="customerAddress-add-button"
            />
          </>
        </div>
      </div>
      <div className="grid align-items-center">
        <div className="col-12" role="customerAddress-datatable">
          <CustomerAddressDatatable
            items={data}
            fields={fields}
...
export default connect(mapState, mapDispatch)(CustomerAddressPage);
