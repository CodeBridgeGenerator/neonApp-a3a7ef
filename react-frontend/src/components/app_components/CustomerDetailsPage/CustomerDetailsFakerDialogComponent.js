...
import customerDetailsFakerFactory from "./customerDetailsFakerFactory";
...
const CustomerDetailsFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = customerDetailsFakerFactory(props.user, fakerCount);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("customerDetails").create(elem),
    );
...
export default connect(
  mapState,
  mapDispatch,
)(CustomerDetailsFakerDialogComponent);
