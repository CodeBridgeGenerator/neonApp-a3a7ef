...
import customerEmailFakerFactory from "./customerEmailFakerFactory";
...
const CustomerEmailFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = customerEmailFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("customerEmail").create(elem));
...
export default connect(mapState, mapDispatch)(CustomerEmailFakerDialogComponent);
