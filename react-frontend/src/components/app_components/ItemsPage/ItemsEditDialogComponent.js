...
const ItemsEditDialogComponent = (props) => {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const eagerResult = await client.service("items").find({
        query: {
          $limit: 10000,
          _id: { $in: [_entity._id] },
          $populate: [
  ...
  const orderOptions = order.map((elem) => ({
    name: elem.name,
    value: elem.value,
  }));
  const productOptions = product.map((elem) => ({
    name: elem.name,
    value: elem.value,
  }));
      ...
      >
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <Dropdown
              id="order"
              value={_entity?.order?._id}
              optionLabel="name"
              optionValue="value"
              options={orderOptions}
              onChange={(e) => setValByKey("order", { _id: e.value })}
            />
          </span>
        ...
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <MultiSelect
              id="product"
              value={_entity?.product?.map((i) => i._id)}
              options={productOptions}
              optionLabel="name"
              optionValue="value"
              onChange={(e) => setValByKey("product", e.value)}
            />
          </span>
          <small className="p-error">
