...
const ItemsFakerDialogComponent = (props) => {
  ...
  useEffect(() => {
    ...
    client
      .service("order")
      .find({ query: { $select: ["_id", "customerName", "total"] } })
      .then((data) => {
        setorderIds(data.data);
      });
  ...
  const onRun = async () => {
    let fakeData = itemsFakerFactory(props.user, fakerCount, orderIds);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("items").create(elem),
    );
...
export default connect(mapState, mapDispatch)(ItemsFakerDialogComponent);
