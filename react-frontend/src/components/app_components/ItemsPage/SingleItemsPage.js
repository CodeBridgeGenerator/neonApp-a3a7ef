...
const [product, setProduct] = useState([]);
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((res) => {
                set_entity(res || {});
                const order = Array.isArray(res.order)
            ? res.order.map((elem) => ({ _id: elem._id, customerName: elem.customerName }))
            : res.order
                ? [{ _id: res.order._id, customerName: res.order.customerName }]
                : [];
        setOrder(order);
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Amount</label><p className="m-0 ml-3" >{Number(_entity?.amount)}</p></div>
            <div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Order</label>
                    {order.map((elem) => (
