...
const OrderHistoryCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            orderNumber: _entity?.orderNumber,orderDate: _entity?.orderDate,totalAmount: _entity?.totalAmount,orderStatus: _entity?.orderStatus,canReorder: _entity?.canReorder || false,variantLabel: _entity?.variantLabel,favourite: _entity?.favourite || false,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Order History created successfully" });
