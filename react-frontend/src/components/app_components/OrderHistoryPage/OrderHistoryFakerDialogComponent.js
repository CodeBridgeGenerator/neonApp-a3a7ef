...
import orderHistoryFakerFactory from "./orderHistoryFakerFactory";
...
const OrderHistoryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = orderHistoryFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("orderHistory").create(elem));
...
export default connect(mapState, mapDispatch)(OrderHistoryFakerDialogComponent);
