...
const OrderHistoryPage = (props) => {
    ...
    client
      ...
      .catch((error) => {
        ...
        props.alert({
          title: "Order History",
          type: "error",
          message: error.message || "Failed get Order History",
        });
      });
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("orderHistory").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  return (
    <div className="mt-5">
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <OrderHistoryEditDialogComponent
