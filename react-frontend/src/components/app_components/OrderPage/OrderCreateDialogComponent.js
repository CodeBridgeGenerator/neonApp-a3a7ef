...
const OrderCreateDialogComponent = (props) => {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const eagerResult = await client.service("order").find({
        query: {
          $limit: 10000,
          _id: { $in: [result._id] },
          $populate: [
      ...
      props.alert({
        type: "error",
        title: "Create",
        message: "Failed to create in Order",
      });
    }
    setLoading(false);
  };
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          $sort: { createdAt: -1 },
          _id: urlParams.singleCustomerDetailsId,
        },
      })
      .then((res) => {
        setCustomerName(
      ...
      .catch((error) => {
        ...
        props.alert({
          ...
          type: "error",
          message: error.message || "Failed get customerDetails",
        });
      });
  }, []);
  ...
  const customerNameOptions = customerName.map((elem) => ({
    name: elem.name,
    value: elem.value,
  }));
  ...
  return (
    ...
    >
      <div
        ...
        style={{ maxWidth: "55vw" }}
        role="order-create-dialog-component"
      >
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <Dropdown
              id="customerName"
              value={_entity?.customerName?._id}
              optionLabel="name"
              optionValue="value"
              options={customerNameOptions}
              onChange={(e) => setValByKey("customerName", { _id: e.value })}
            />
          </span>
        ...
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <InputText
              ...
              value={_entity?.total}
              onChange={(e) => setValByKey("total", e.target.value)}
            />
          </span>
          <small className="p-error">
