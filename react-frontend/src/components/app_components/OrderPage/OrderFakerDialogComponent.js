...
const OrderFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("customerDetails")
                        .find({ query: { $select: ["_id","customerName","customerEmail","customerAddress","phoneNumber"] } })
                        .then((data) => {
    ...
    const onRun = async () => {
        let fakeData = orderFakerFactory(props.user,fakerCount,customerNameIds);
...
export default connect(mapState, mapDispatch)(OrderFakerDialogComponent);
