...
import orderProductFakerFactory from "./orderProductFakerFactory";
...
const OrderProductFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = orderProductFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("orderProduct").create(elem));
...
export default connect(mapState, mapDispatch)(OrderProductFakerDialogComponent);
