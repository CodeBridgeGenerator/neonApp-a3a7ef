...
import productColorFakerFactory from "./productColorFakerFactory";
...
const ProductColorFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = productColorFakerFactory(props.user, fakerCount);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("productColor").create(elem),
    );
...
export default connect(mapState, mapDispatch)(ProductColorFakerDialogComponent);
