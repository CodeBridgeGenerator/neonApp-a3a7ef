...
import ProductColorDatatable from "./ProductColorDataTable";
...
import ProductColorFakerDialogComponent from "./ProductColorFakerDialogComponent";
import ProductColorSeederDialogComponent from "./ProductColorSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const ProductColorPage = (props) => {
  ...
  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("productColor");
    ...
    client
      ...
      .catch((error) => {
        ...
        props.alert({
          title: "Product Color",
          type: "error",
          message: error.message || "Failed get Product Color",
        });
      });
  ...
  const deleteRow = async () => {
    try {
      await client.service("productColor").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("productColor").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  const onRowClick = ({ data }) => {
    navigate(`/productColor/${data._id}`);
  };
  ...
  return (
    <div className="mt-5">
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="productColor-add-button"
            />
          </>
        </div>
      </div>
      <div className="grid align-items-center">
        <div className="col-12" role="productColor-datatable">
          <ProductColorDatatable
            items={data}
            fields={fields}
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <ProductColorEditDialogComponent
...
export default connect(mapState, mapDispatch)(ProductColorPage);
