...
const ProductCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            productTitle: _entity?.productTitle,description: _entity?.description,price: _entity?.price,productImage: _entity?.productImage,smallImage: _entity?.smallImage,
            ...
            .find({ query: { $limit: 10000 ,  _id :  { $in :[result._id]}, $populate : [
                {
                    ...
                    select:["productName","basePrice","currency","discountedPrice","taxPercentage"]}
            ] }});
        props.onHide();
    ...
    useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setPrice(res.data.map((e) => { return { productName: `${e["productName"]}`,basePrice: `${e["basePrice"]}`,currency: `${e["currency"]}`,discountedPrice: `${e["discountedPrice"]}`,taxPercentage: `${e["taxPercentage"]}`, value: e._id }}));
                        })
                        .catch((error) => {
                            console.debug({ error });
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <MultiSelect id="price" value={_entity?.price} options={priceOptions} optionLabel="name" optionValue="value" onChange={(e) => setValByKey("price", e.value)}  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText className="w-full mb-3 p-inputtext-sm" value={_entity?.smallImage} onChange={(e) => setValByKey("smallImage", e.target.value)}  />
            </span>
            <small className="p-error">
