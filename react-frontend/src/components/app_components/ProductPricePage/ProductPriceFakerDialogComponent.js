...
import productPriceFakerFactory from "./productPriceFakerFactory";
...
const ProductPriceFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = productPriceFakerFactory(props.user, fakerCount);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("productPrice").create(elem),
    );
...
export default connect(mapState, mapDispatch)(ProductPriceFakerDialogComponent);
