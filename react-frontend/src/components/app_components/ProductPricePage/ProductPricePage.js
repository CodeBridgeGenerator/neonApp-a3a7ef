...
import ProductPriceDatatable from "./ProductPriceDataTable";
import ProductPriceEditDialogComponent from "./ProductPriceEditDialogComponent";
import ProductPriceCreateDialogComponent from "./ProductPriceCreateDialogComponent";
import ProductPriceFakerDialogComponent from "./ProductPriceFakerDialogComponent";
import ProductPriceSeederDialogComponent from "./ProductPriceSeederDialogComponent";
...
const ProductPricePage = (props) => {
  ...
  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("productPrice");
    ...
    client
      ...
      .catch((error) => {
        ...
        props.alert({
          title: "Product Price",
          type: "error",
          message: error.message || "Failed get Product Price",
        });
      });
  ...
  const deleteRow = async () => {
    try {
      await client.service("productPrice").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("productPrice").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  const onRowClick = ({ data }) => {
    navigate(`/productPrice/${data._id}`);
  };
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="productPrice-add-button"
            />
          </>
        </div>
      </div>
      <div className="grid align-items-center">
        <div className="col-12" role="productPrice-datatable">
          <ProductPriceDatatable
            items={data}
            fields={fields}
...
export default connect(mapState, mapDispatch)(ProductPricePage);
