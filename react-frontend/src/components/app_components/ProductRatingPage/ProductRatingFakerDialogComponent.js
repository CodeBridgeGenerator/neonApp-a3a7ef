...
import productRatingFakerFactory from "./productRatingFakerFactory";
...
const ProductRatingFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = productRatingFakerFactory(props.user, fakerCount);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("productRating").create(elem),
    );
