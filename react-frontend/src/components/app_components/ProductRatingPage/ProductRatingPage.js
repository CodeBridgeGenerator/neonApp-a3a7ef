...
import ProductRatingDatatable from "./ProductRatingDataTable";
import ProductRatingEditDialogComponent from "./ProductRatingEditDialogComponent";
import ProductRatingCreateDialogComponent from "./ProductRatingCreateDialogComponent";
import ProductRatingFakerDialogComponent from "./ProductRatingFakerDialogComponent";
import ProductRatingSeederDialogComponent from "./ProductRatingSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const ProductRatingPage = (props) => {
  ...
  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("productRating");
    ...
    client
      ...
      .catch((error) => {
        ...
        props.alert({
          ...
          type: "error",
          message: error.message || "Failed get Product Rating",
        });
      });
  ...
  const deleteRow = async () => {
    try {
      await client.service("productRating").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("productRating").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  const onRowClick = ({ data }) => {
    navigate(`/productRating/${data._id}`);
  };
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="productRating-add-button"
            />
          </>
        </div>
      </div>
      <div className="grid align-items-center">
        <div className="col-12" role="productRating-datatable">
          <ProductRatingDatatable
            items={data}
            fields={fields}
...
export default connect(mapState, mapDispatch)(ProductRatingPage);
