...
import productSizeFakerFactory from "./productSizeFakerFactory";
...
const ProductSizeFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = productSizeFakerFactory(props.user, fakerCount);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("productSize").create(elem),
    );
...
export default connect(mapState, mapDispatch)(ProductSizeFakerDialogComponent);
