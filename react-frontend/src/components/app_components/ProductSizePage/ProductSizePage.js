...
import ProductSizeDatatable from "./ProductSizeDataTable";
...
import ProductSizeFakerDialogComponent from "./ProductSizeFakerDialogComponent";
import ProductSizeSeederDialogComponent from "./ProductSizeSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const ProductSizePage = (props) => {
  ...
  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("productSize");
    ...
    client
      ...
      .catch((error) => {
        ...
        props.alert({
          title: "Product Size",
          type: "error",
          message: error.message || "Failed get Product Size",
        });
      });
  ...
  const deleteRow = async () => {
    try {
      await client.service("productSize").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("productSize").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="productSize-add-button"
            />
          </>
        </div>
      </div>
      <div className="grid align-items-center">
        <div className="col-12" role="productSize-datatable">
          <ProductSizeDatatable
            items={data}
            fields={fields}
...
export default connect(mapState, mapDispatch)(ProductSizePage);
