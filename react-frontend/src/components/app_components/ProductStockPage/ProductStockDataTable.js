...
const ProductStockDataTable = ({
  ...
  const p_numberTemplate2 = (rowData, { rowIndex }) => (
    <p>{rowData.quantityAvailable}</p>
  );
  const p_dateTemplate3 = (rowData, { rowIndex }) => (
    <p>{moment(rowData.lastRestockedDate).fromNow()}</p>
  );
  const multiselectTemplate4 = (rowData, { rowIndex }) => (
    <ul className="list-none">
      {rowData.products.map((i, x) => (
        <li key={x}>
          {" "}
          productName: {i.productName},sizeCategory: {i.sizeCategory},sizeValue:{" "}
          {i.sizeValue},stockQuantity: {i.stockQuantity},availableSize:{" "}
          {i.availableSize}
        </li>
      ))}
    </ul>
    ...
    <>
      ...
      >
        ...
        <Column
          ...
          filter={selectedFilterFields.includes("quantityAvailable")}
          hidden={selectedHideFields?.includes("quantityAvailable")}
          sortable
          style={{ minWidth: "8rem" }}
        />
        <Column
          field="lastRestockedDate"
          header="Last Restocked Date"
          body={p_dateTemplate3}
          filter={selectedFilterFields.includes("lastRestockedDate")}
          hidden={selectedHideFields?.includes("lastRestockedDate")}
          sortable
          style={{ minWidth: "8rem" }}
        />
        <Column
      ...
      <Dialog
        header="Upload ProductStock Data"
        visible={showUpload}
