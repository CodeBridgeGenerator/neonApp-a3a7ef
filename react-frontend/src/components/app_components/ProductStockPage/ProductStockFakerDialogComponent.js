...
import productStockFakerFactory from "./productStockFakerFactory";
...
const ProductStockFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = productStockFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("productStock").create(elem));
...
export default connect(mapState, mapDispatch)(ProductStockFakerDialogComponent);
