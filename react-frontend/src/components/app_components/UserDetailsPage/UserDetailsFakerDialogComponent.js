...
const UserDetailsFakerDialogComponent = (props) => {
  ...
  useEffect(() => {
    ...
    client
      ...
      .find({ query: { $select: ["_id", "email"] } })
      .then((data) => {
        setemailIds(data.data);
      });
  ...
  const onRun = async () => {
    let fakeData = userDetailsFakerFactory(props.user, fakerCount, emailIds);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("userDetails").create(elem),
    );
