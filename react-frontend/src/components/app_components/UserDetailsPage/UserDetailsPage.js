...
const UserDetailsPage = (props) => {
  ...
  useEffect(() => {
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          email: urlParams.singleCustomerEmailId,
          $populate: [
            {
              path: "createdBy",
              service: "users",
      ...
      .catch((error) => {
        ...
        props.alert({
          title: "User Details",
          type: "error",
          message: error.message || "Failed get User Details",
        });
      });
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("userDetails").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
