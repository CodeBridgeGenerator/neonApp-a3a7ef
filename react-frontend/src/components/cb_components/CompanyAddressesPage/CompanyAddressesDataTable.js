...
const CompanyAddressesDataTable = ({
  ...
  const p_booleanTemplate8 = (rowData, { rowIndex }) => (
  ...
  const confirmDelete = async () => {
    try {
      const promises = selectedItems.map((item) =>
        client.service("companyAddresses").remove(item._id),
      );
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      toast.current.show({
        severity: "success",
        summary: "Copied",
        detail: `companyAddresses data copied to clipboard`,
        life: 3000,
      });
      deselectAllRows();
      setRefresh((prev) => !prev);
  ...
  const handleDuplicate = async () => {
    ...
    try {
      const promises = selectedItems.map((item) => {
        ...
        return client.service("companyAddresses").create(newItem);
      });
      const createdItems = await Promise.all(promises);
      toast.current.show({
        severity: "success",
        summary: "Duplicated",
        detail: `${selectedItems.length} companyAddresses duplicated successfully`,
        life: 3000,
      });
      deselectAllRows();
      setRefresh((prev) => !prev);
    } catch (error) {
      console.error("Failed to duplicate companyAddresses", error);
      toast.current.show({
        severity: "error",
        summary: "Error",
        detail: "Failed to duplicate companyAddresses",
        life: 3000,
      });
    }
  };
