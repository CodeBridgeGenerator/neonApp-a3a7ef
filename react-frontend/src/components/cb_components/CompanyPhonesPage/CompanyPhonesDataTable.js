  ...
  const confirmDelete = async () => {
    try {
      const promises = selectedItems.map((item) =>
        client.service("companyPhones").remove(item._id),
      );
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      toast.current.show({
        severity: "success",
        summary: "Copied",
        detail: `companyPhones data copied to clipboard`,
        life: 3000,
      });
      deselectAllRows();
      setRefresh((prev) => !prev);
  ...
  const handleDuplicate = async () => {
    ...
    try {
      const promises = selectedItems.map((item) => {
        ...
        return client.service("companyPhones").create(newItem);
      });
      const createdItems = await Promise.all(promises);
      toast.current.show({
        severity: "success",
        summary: "Duplicated",
        detail: `${selectedItems.length} companyPhones duplicated successfully`,
        life: 3000,
      });
      deselectAllRows();
      setRefresh((prev) => !prev);
    } catch (error) {
      console.error("Failed to duplicate companyPhones", error);
      toast.current.show({
        severity: "error",
        summary: "Error",
        detail: "Failed to duplicate companyPhones",
        life: 3000,
      });
    }
  };
...
export default connect(mapState, mapDispatch)(CompanyPhonesDataTable);
